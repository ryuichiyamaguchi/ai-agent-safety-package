...
test('キー登録ボタンが両 OS にあり、取り消せない操作だと伝えている', () => {
  // v1.18.0: キー系ボタンはサブフォルダ「キーと金庫」へ移った（5 番）。
  const cmd = read('workspace-template/スタート/キーと金庫/5_Bufferのキーを登録.command');
  ...
  const bat = fs.readFileSync(path.join(root, 'workspace-template/スタート/キーと金庫/5_Bufferのキーを登録.bat'));
