...
if [ "${DS_CLAUDE_MODE:-}" = "1" ]; then
  ...
  _use_search=0; _use_image=0; _use_agy=0
  ...
  if [ $((_use_search + _use_image + _use_agy)) -gt 0 ] \
    ...
    # JSON はパスのエスケープ事故を避けるため node で書き出す（d-claude 経路では node 必須）。
    # 引数: 出力先, search(js or ""), image(js or ""), agy(js or "")
    if node -e '
      const fs=require("fs");
      const servers={};
      ...
      if(process.argv[5]) servers["agy-image"]={command:"node",args:[process.argv[5]]};
      fs.writeFileSync(process.argv[2],JSON.stringify({mcpServers:servers}));
    ' "$_mcp_cfg" "$([ $_use_search -eq 1 ] && printf '%s' "$_search_mcp")" "$([ $_use_image -eq 1 ] && printf '%s' "$_image_mcp")" "$([ $_use_agy -eq 1 ] && printf '%s' "$_agy_mcp")" 2>/dev/null; then
