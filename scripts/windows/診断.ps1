...
function Show-PermissionRepairHint([string]$Dir) {
    ...
    Line ""
    Line "       (A) スタート フォルダの「12_フォルダのアクセス権を直す」を実行する ←おすすめ"
    Line ""
...
if (-not $profileFound) {
    ...
    Line "    ● 対処: 退治スクリプトでバックアップ付きコメントアウトを実行するか、"
    Line "       上の該当行を手でコメントアウトして、新しいターミナルを開き直してください。"
    Line "       退治スクリプト（PowerShell で実行）:"
    Line ("         powershell -NoProfile -ExecutionPolicy Bypass -File `"" + (Join-Path $Workspace ".ai-safety\hooks\windows\野良d-claudeを退治.ps1") + "`" -Workspace `"" + $Workspace + "`"")
}
...
Line "============================================================"
if ($permBroken) {
    Line "  ★ 最優先: 「12_フォルダのアクセス権を直す」を実行してください。"
