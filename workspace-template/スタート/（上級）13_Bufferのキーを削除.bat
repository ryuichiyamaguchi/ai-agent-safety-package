...
REM （上級）13_Bufferのキーを削除.bat
REM 登録した Buffer の API キー を、この PC から消します。
REM ・Windows の金庫(DPAPI)のファイルと、移行前の平文ファイルの両方を消します。
REM ・PC 側を消しても、発行元のサイトではキーが生きています。

echo.
echo  Buffer の API キー をこの PC から削除します...
del /f /q "%USERPROFILE%\.ai-safety\buffer.dpapi" >nul 2>&1
del /f /q "%USERPROFILE%\.ai-safety\buffer-api-key.txt" >nul 2>&1
