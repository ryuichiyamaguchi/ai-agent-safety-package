#!/bin/bash
# 4_AIコーチのキーを削除.command
# 登録した AIコーチ（Gemini）の API キー を、この Mac から消します。
# ・Mac の金庫（キーチェーンの「ai-safety.gemini」）と、移行前の平文ファイルの両方を消します。
# ・PC 側を消しても、発行元のサイトではキーが生きています。
#   Google AI Studio（https://aistudio.google.com/apikey）でも、そのキーを削除してください。
set -u
...
echo ""
echo " AIコーチ（Gemini）の API キー をこの Mac から削除します..."
