...
- 置き場は**ワークスペース直下**の `.ai-safety/plugins/`。その直下に `.js` / `.ts` を置きます（案件ごとのフォルダに作っても読み込みません）
...
- このフォルダは**AIには作れません**（`.ai-safety` への書き込みは禁止しているため）。裏を返せば、AIが勝手にプラグインを仕込むことはできません
