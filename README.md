...
- **「長時間おまかせモード」を新設**しました（Mac の Claude のみ）。目を離して作業を任せるためのモードで、OS の壁の中でだけ確認を省きます
...
更新のしかたは変わりません。「スタート」フォルダの **「7_安全パッケージを最新版に更新」→「8_AIツールを最新版に更新」** の順に実行してください。
...
⚠️ **この送信検査 Gateway は「DeepSeek に送る文字列の検査」であって、PC から外に出る通信を全部通す関所ではありません。** `gemini-search` / `gemini-vision` / `pollinations-image` / `agy-image` / `buffer` の 5 本の MCP、AI の `WebFetch`、シェルから直接打たれた `curl` / `wget` は、**Gateway を通らずに外へ出ます**。
