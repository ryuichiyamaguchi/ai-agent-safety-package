...
- 「スタート」フォルダに **「（上級）12_プラグインの置き場を開く」** を追加しました。ダブルクリックすると置き場が開きます
- 既に使っている方は「6_最新版に更新」を実行すると置き場ができます
...
- 置き場は**ワークスペース直下**の `.ai-safety/plugins/`。その直下に `.js` / `.ts` を置きます（案件ごとのフォルダに作っても読み込みません）。v1.15.1 からは導入時に用意され、「スタート」の**「（上級）12_プラグインの置き場を開く」**で開けます
