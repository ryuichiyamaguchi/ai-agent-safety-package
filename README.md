...
特に 4 の `approval_policy = "untrusted"` は v1.0.8 で導入した中核の防御です。`python -c "open('.env')..."`、`curl https://attacker/`、`rm -rf`、`git push --force` などはこの層で止まります。
