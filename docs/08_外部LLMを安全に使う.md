...
本パッケージの守り（壁・見張り・記録係）は「AI ツールが**ローカル PC で何をするか**」を縛るのが目的です。一方、外部 LLM を使うときに新しく出てくる問題は「あなたが書いたプロンプトの**中身がどこに送られるか**」という別軸の話で、壁も見張りも対象外です。
...
| 担当者 | 壁（OS サンドボックス）+ 見張り（hook・権限リスト）+ 記録係 | あなた自身 + secret-scan（マスキング） |
...
v1.6.0 で追加。`--auto` を付けて起動すると、承認プロンプトを減らして AI に自走させます。起動時に「壁（OS サンドボックス）がどこまで効くか」を診断して**画面に表示します**が、承認を減らすかどうかは診断結果によらず `--auto` の指定で決まります。危険なコマンドは、承認とは関係なく**見張り（hook）が止めます**。
...
| エンジン | 壁の強さ | 起動時にすること | 承認モード |
|---|---|---|---|
| **Codex** | **強**（Mac / Windows とも壁あり） | 壁が「作業フォルダ外に書けない」状態を作れるか実際に試し、結果を画面に表示 | `--ask-for-approval on-failure` |
| **agy** | **弱・宣言ベース** | agy バイナリの存在確認のみ（壁を外から実証する手段がないため） | `--dangerously-skip-permissions`（`--sandbox` は維持） |
| **Claude Code** | 対象外 | — | — |

### 重要な制限事項

- **`--auto` でも通信は止まりません。** Codex の設定は `network_access = true`（通信を許可）です。調べもの・パッケージ取得・API の動作確認といった実用を止めないための判断です。外への持ち出しは、匿名アップロード先の拒否や秘密パターンの検出といった**見張り側**が担当します。
- 起動時の診断で「壁が確認できなかった」と表示されても、自走そのものは続きます。危険なコマンドは承認とは無関係に**見張り（hook）が止める**ためです。表示は「いま壁がどこまで効いているか」を隠さず伝えるためのものです。
- **agy の壁は未実証**です。`--sandbox` フラグを信頼するもので、Codex のように独立した検証はしていません。重要な作業では手動承認の利用も検討してください。
- `--auto` にしても壁と見張りは常時稼働します（省くのは承認の手間だけです）。

### トラブルシューティング

起動時に「壁を確認できませんでした」という趣旨の警告が出た場合は、`doctor.sh`（macOS）または `doctor.ps1`（Windows）を実行して、壁の状況を確認してください。警告が出ていても見張りは動いています。

## 関連ドキュメント

- `docs/00_はじめに.md` — 「壁・見張り・記録係」の言葉の意味
- `docs/13_秘密の入れ物-APIキーの安全な持ち方.md` — API キーを安全に持つ方法
- `docs/90_守れる-守れない.md` — 守れる範囲・守れない範囲の全リスト
