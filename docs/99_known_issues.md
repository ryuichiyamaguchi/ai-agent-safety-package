...
1. **「スタート」フォルダの `12_フォルダのアクセス権を直す` を実行する**（おすすめ・ボタン 1 つ）
...
- **`takeown` は既定で実行しません。** 実機では「アクセスが拒否されました」が大量発生し、しかも不要でした。
  ...
  （`install.ps1` も `12_フォルダのアクセス権を直す` も渡しません）。
...
- 実体は `scripts/windows/repair-permissions.ps1` に集約し、導入時（`install.ps1` から `-Quiet -NoTakeown`）と
  修復ボタン（`12_フォルダのアクセス権を直す`）が**同じコード**を通ります。
...
- 壊れた実機で `12_フォルダのアクセス権を直す` **ボタン**を押して回復すること
