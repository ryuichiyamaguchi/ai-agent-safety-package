...
### v1.15.2（現行HEAD、ダッシュボードから雇うための裏実行モード）
...
> v1.14.18 からの変更(2): **`oc-safe` が「いま居る場所」から作業フォルダを判定するようにした**。これまでは導入時に焼き込んだパスだけを見ていたため、**検証用フォルダへ導入し直すと焼き込み値がそちらへ書き換わり、本来の作業フォルダの中に居るのに「作業フォルダの中で使ってください」と断られる**事故が起きた（実機で発生）。対策= 現在地から上へ `.ai-safety/hooks/macos/launch-integrated.sh` を探し、見つかったフォルダを作業フォルダとする。焼き込み値は見つからなかったときのフォールバックに降格。これで複数の作業フォルダを使い分けても「居る側」で正しく動き、焼き込みが古くても壊れない。どの作業フォルダの外でもないときは従来どおり断る。検証= 作業フォルダを 2 つ作り、焼き込みが片方を指している状態でもう片方の案件フォルダから起動して、居る側が使われることを実走確認。
> v1.14.18 からの変更: **★v1.14.18 の修正が効いていなかったので本修正**。v1.14.18 は「`plutil -p` は標準入力が駄目でファイル指定なら通る」という前提で一時ファイル経由に切り替えたが、これは報告の誤読だった。実機で確認した結果は **`plutil -p /tmp/t.json` もファイル指定で失敗**（`-p` 自体が JSON 非対応）で、通るのは `plutil -convert` だけ。つまり v1.14.18 は何も変えていなかった。対策= **`plutil -p` への依存そのものを外す**。Node（送信検査 Gateway 等で元から必須）で同じ見た目のテキストを生成する `scripts/common/plutil-p.js` を新設し、`plutil -p` が失敗した環境ではこちらで読み直す。出力形式は本物と**完全一致**することを回帰テストで固定した（辞書はキーをソート／配列は添字付き／インデント 2／空の配列と辞書は括弧のみ／**文字列はエスケープせず生**／null は `<null>`）ので、deny 床の照合は一切変わらない。検証= 「`-p` を常に失敗させる plutil」で実機を忠実に再現し、①無害な「こんにちは」が通る（症状解消）②`rm -rf /Users/example/Documents` は exit=2 で止まる③無害な `ls -la` は通る ④本物の `plutil -p` と Node 実装の出力が 3 種類の入力で一致（引用符・バックスラッシュ・改行・タブ・日本語・null・小数・負数・空配列/空辞書を含む）⑤壊れた JSON は読めないまま（fail-closed 維持）を実走確認。
> v1.14.17 からの変更: **★macOS 14 (Sonoma) で Claude Code が一切使えなくなる欠陥を修正**。ガードはフック入力(JSON)を `plutil -p -`（標準入力）で読むが、**Sonoma ではこれが必ず "Unexpected character { at line 1" で失敗する**（受講者の実機で判明。当事者の Claude が原因を特定）。読めない＝検査できないので設計どおり fail-closed になり、**無害なプロンプトまで含めて全部ブロック**され「AI Safety Guard BLOCKED: 入力データを読み取れなかったため、念のため実行を止めました。」だけが出続けていた。macOS 26 では `plutil -p -` が通るため講師機では再現しない（OS 差）。対策= 同じ JSON でも**ファイル指定なら読める**ので、標準入力で失敗したときは一時ファイル（0600・成功失敗どちらでも削除）経由で読み直す。出力は同じ `plutil -p` 形式なので、以降の検査（deny 床の照合）は一切変わらない。検証= 「標準入力だけ失敗する plutil」で Sonoma を再現し、①修正前は無害な「こんにちは」で exit=2（受講者の症状を再現）②修正後は exit=0 で通る ③それでも `rm -rf /Users/example/Documents` は exit=2 で止まる④無害な `ls -la` は通る、を実走確認。回帰テストも同じ再現ハーネスで自動化した。
> v1.14.16 からの変更: **「Bouncer統合版を起動」から OpenCode を選んだとき、作業フォルダを番号で選べるようにした**。OpenCode は起動したフォルダが作業対象になるため（本体仕様。動き出したあとで cd しても移らない）、案件ごとにフォルダを分けて作業するには起動時に決める必要がある。`oc-safe` では指定できたが、ボタンから起動する受講者には手段が無かった。対策= メニューで OpenCode（通常／Web検索／続きから の 3 経路すべて）を選ぶと、**作業フォルダ直下のフォルダ一覧を番号で提示**し、選んだものを `--project=` / `-Project` で渡す。`0` または未入力なら従来どおり作業フォルダ直下で起動する（パスは打たせない）。隠しフォルダ・`スタート`・`safe-workspace` は候補から除く。⚠️実装の罠 2 件: ①macOS の bash 3.2 は**変数の直後に日本語が続くと変数名の切れ目を取り違える**ため `「${_sel}」` のように `${}` で囲む必要がある（`「$_sel」` は unbound variable で落ちる。実測）②cmd は**変数名の中で番号を展開できない**ため `call set "PSEL=%%PDIR_!PICK!%%"` と call を挟む。
> v1.14.15 からの変更: **Buffer（SNS の予約投稿サービス）を OpenCode の MCP として追加**。これまで MCP はローカルの node スクリプト 4 本（Gemini 検索／画像読取／画像生成 2 種）だけで、ホスト型（リモート）の MCP を扱う口が無かった。Buffer は公式のホスト型 MCP を提供しているため、`type: "remote"` で接続する経路を新設した（`REMOTE_MCP_SERVERS`）。接続先= `https://mcp.buffer.com/mcp`・認証= `Authorization: Bearer <APIキー>`・`oauth: false`（自動 OAuth でブラウザが開いて止まるのを防ぐ）。**鍵が無ければ登録しない**（Gemini 系と同じ条件付き登録。鍵なしで登録すると毎回 401 を返すツールが並ぶため）。鍵の置き場は `~/.ai-safety/buffer-api-key.txt`（0600）で、「（上級）11_Bufferのキーを登録」ボタンを両 OS に追加。`AI_SAFE_DCLAUDE_BUFFER=0` で個別無効化。**権限は `buffer_*: ask`**＝ SNS への投稿は取り消せないため、実行前に必ず確認を挟む（既定の `*: ask` に頼らず設定にも明示）。また解決済み設定には鍵が Authorization ヘッダとして載るので、**診断ファイルへ書く前に鍵を伏せる処理を両 OS のランチャーに追加**（gateway の合言葉と同じ扱い）。⚠️MCP の通信は OpenCode から先方へ直接出るため、DeepSeek 向けの送信検査（マスク）は経由しない（既存の Gemini 検索 MCP と同じ構造）。
> v1.14.14 からの変更: **Windows 配布の `ccmux.exe` を、上位階層移動入りのビルドへ差し替え**。v1.14.14 では mac 版だけ先行し、Windows は前回ビルド（ドラッグ&ドロップと Shift+ホイールのみ）のままだった。fork の push で GitHub Actions の Windows ビルドが自動実行されており（run 31312693334・cargo test 込みで success）、その成果物へ差し替えた。新 SHA-256 = `a756925c…`（旧 `4b200120…`）。`install-ccmux.ps1` の期待値も同時に更新。これで mac / Windows とも `h`・←・Backspace で親フォルダへ、`l`・→ で選択フォルダへ入れる。
> v1.14.13 からの変更: **mac にも改造版 ccmux を配布し、ファイルツリーで上の階層へ移動できるようにした**。これまで mac の「（上級）10_ccmuxを入れる」は**本家の npm 版を入れるだけ**で、改造版（ファイルツリーからのドラッグ&ドロップでパス貼り付け・Shift+ホイールでの遡り）は Windows 用 `.exe` しか配布していなかった。さらに **上位階層への移動はどちらにも実装されていなかった**（fork の `move_up`/`move_down` はカーソル移動であって親フォルダへは行けない。旧改造版にあった機能はソース消失で失われたまま）。対策= ①fork（`ryuichiyamaguchi/ccmux`）に上位階層移動を実装（`h`/←/Backspace で親へ・`l`/→ で選択フォルダへ入る・ペインのタイトルに現在フォルダと操作キーを表示。cargo test 39/39）②mac 用 arm64 バイナリを Release へ添付し、`.command` が SHA-256 照合してから `~/.ai-safety/bin/ccmux` へ配置（検疫属性も外す）。Intel Mac は本家 npm 版にフォールバック。**プレビュー（中身表示）は従来どおり起動フォルダの中に限定**したままなので、上へ移動しても外のファイルの中身は開けない（名前とパスだけ見えて D&D に使える）。MIT ライセンスの改変内容の記載も追従させた。
> v1.14.12 からの変更: **★`oc-safe` が見守りモニター（Bouncer の画面）を起動していなかった欠陥を修正**。v1.14.12 の `oc-safe` は OpenCode のランチャーを直接呼んでいたが、モニターを立ち上げるのは統合ランチャー（`launch-integrated.{sh,ps1}`）の役目だったため、`oc-safe` で起動すると**画面で見えないまま AI が動く**状態になっていた（実機で発覚。危険コマンドを止めるOpenCode 側のプラグインと deny 床は効いていたが、モニター画面と AI コーチが使えない）。対策= `oc-safe` は必ず統合ランチャーを経由する（mac は `--project=<フォルダ>`、Windows は`-Project` で作業フォルダを渡す）。統合ランチャーの dry-run に `session:` と `project:` を表示し、経由しているかをテストで検証できるようにした。検証= node tests 279/279、mac install+doctor pass=47 fail=0。回帰テスト追加= `oc-safe` が統合ランチャーを呼ぶこと・OpenCode のランチャーを直接叩かないこと・`Bouncer統合版` と `monitor: enabled` が出ること（両 OS）。
> v1.14.11 からの変更: **`oc-safe` を追加 = 作業フォルダの中のプロジェクトごとに OpenCode を起動できるコマンド**。OpenCode は「起動したフォルダ」が作業対象になり、動き出したあとで `cd` しても移らない（本体仕様。公式ドキュメントも「起動前に cd してから起動」と案内）。そのため「my-ai-workspace の中に案件ごとのフォルダを作って、その中で作業する」には、そのフォルダで起動する必要がある。また ccmux / Zed のターミナルから Bouncer 監視下の OpenCode を 1 行で起動する導線が無かった。対策= ①ランチャー（mac `--project` / Windows `-Project`）が起動フォルダを受け取り、そこへ移ってからOpenCode を起動する ②`oc-safe` を PATH に通す（mac: install が `~/.ai-safety/bin/oc-safe` を生成しワークスペースを焼き込み、`~/.zshrc` に PATH を冪等追加／Windows: `setup-commands.ps1` のシム一覧へ追加）。使い方は `oc-safe`（いま開いているフォルダ）・`oc-safe 案件A`（ワークスペース内の同名フォルダ）・`oc-safe 案件A --resume`。**ワークスペースの外は断る**（ガードとポリシーはワークスペース基準で配置されているため、外で起動すると保護が及ばない場所で AI が動く）。判定は物理パス（`pwd -P`）で行う＝ macOS の `/var`→`/private/var` のようなシンボリックリンクが途中にあると論理パスのままでは「中にあるのに外」と誤判定されるため。検証= install 実走で配置・PATH 追加・2 回目の冪等性を確認、フォルダ名指定／いる場所／`--resume` 併用／ワークスペース外の拒否を実走確認。
