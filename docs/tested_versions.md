...
1. スタート「7_安全パッケージを最新版に更新」→「8_AIツールを最新版に更新」の順に実行
2. スタート「9_困ったとき診断」を再実行
...
### v1.17.0（現行HEAD、卒業後の実用に耐える最小限の安全網へ — 純正サンドボックス・秘密の金庫・PC全体設定の4エンジン対応・docs 全面見直し）
...
> v1.15.3（OpenCode に Playwright MCP を追加）からの変更: **卒業後最終形としてパッケージ全体を整理した。安全規則（policy の deny 床・guard・判定ロジック）は不変。** (1) **docs の全面再構成**: 既存ドキュメントの整理・番号再編に加えて新設 3 本（`docs/11_AGI-Cockpitで使う.md` / `docs/20_卒業後ガイド.md` / `docs/91_やさしい仕組み解説.md`）を追加。(2) **スタートフォルダの番号再編**: ボタンの並びを用途順に振り直した。(3) **「8_AIツールを最新版に更新」ボタンを新設**（`update-ai-tools.{sh,ps1}`）。(4) **workspace への docs 同期**: install が docs をワークスペースへ同期し、卒業後も手元で参照できるようにした。(5) **d-claude は維持**しつつ卒業後の移行案内を追記。(6) **ccmux を廃止**。
...
| scripts/common/secret-store.js | 5d965313381ca9913bf8443e8c98d626b3fee2c9a3d766ce6d2be78edd1b1d33 | 2026-08-20 配布前修正: 起動スクリプトが「鍵が登録されているか」だけを聞ける最小 CLI（`--has <name>`）を追加。判定は resolve()（環境変数 → 金庫 → 旧平文）そのもので、値は出力しない。旧hash=7b1acaeb…。 v1.17.0 新規: 秘密の金庫（OS のキーチェーン / DPAPI へ API キーを預ける）。**平文ファイルへのフォールバックを含む中核なので改ざん検知の対象に入れた。** |
| scripts/common/secret-migrate.js | 2d857c5f105886576c7cbe3371356a3a23d60dc13ae27c57b8eb4829583ac768 | v1.17.0 新規: 既存の平文キーを金庫へ移す（受講生の操作ゼロ）。移行元の削除を伴うため改ざん検知の対象。 |
...
| scripts/macos/launch-claude-longrun.sh | d8584e781f8537bd8217331cbf419ef22cc3b6d281942e9af20a578a10e4df45 | 2026-08-20 配布前修正: このモードの**一時設定にだけ** `sandbox.failIfUnavailable: true` を追加。従来は settings の `sandbox.enabled` という宣言しか見ておらず、Seatbelt の起動に失敗しても ask 空・acceptEdits のまま壁なしで走り得た。Claude Code 2.1.236 のバイナリ内文字列（`Sandbox required but unavailable:` / `Set sandbox.failIfUnavailable=false to allow unsandboxed execution.`）で挙動を確認済み。恒久設定（configs/claude/settings.mac.json）には入れない。旧hash=2d46d68d…。 v1.17.0 新規: 「（上級）15_長時間おまかせモードで起動」の mac 実体。**承認を省く代わりに壁（sandbox）必須・deny 床維持・bypassPermissions 封印**という条件をこのファイルが担保するため、改ざん検知の対象。 |
| scripts/windows/launch-claude-longrun.ps1 | 1805c5ac3ea22ea640baf957bc5b65af44517440138ba169e9fcd8cbeb460fe0 | v1.17.0 新規: 同上の Windows 側。Windows では壁が使えないため**起動を拒否する**のが仕様（BOM 付き UTF-8 / CRLF）。 |
