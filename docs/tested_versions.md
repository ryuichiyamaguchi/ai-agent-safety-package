...
### v1.14.3（現行HEAD、OpenCode初回準備ログと設定JSONの安全な分離）
...
> 個別ファイルの SHA-256 一覧は、現行版である上の v1.14.3 節の表に集約した（v1.14.0 時点の値は git 履歴を参照）。
