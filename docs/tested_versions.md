...
### v1.14.2（現行HEAD、OpenCode依存キャッシュ誤検出修正＋更新完了版の自己確認）
...
> 個別ファイルの SHA-256 一覧は、現行版である上の v1.14.2 節の表に集約した（v1.14.0 時点の値は git 履歴を参照）。
