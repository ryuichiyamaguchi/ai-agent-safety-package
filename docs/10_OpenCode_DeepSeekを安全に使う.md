...
1. **ワークスペース（例: `~/Documents/my-ai-workspace`、Windowsは `%USERPROFILE%\Documents\my-ai-workspace`）の直下**に `.ai-safety/plugins/`（Windowsは `.ai-safety\plugins\`）というフォルダを作る
2. その**直下**にプラグインのファイル（`.js` または `.ts`）を置く。フォルダの中にさらにフォルダを作っても読み込みません
3. いつもどおり統合ランチャーからOpenCodeを起動する

> **場所を間違えやすい点が2つあります。**
> - 案件ごとのフォルダ（`my-ai-workspace/myproject/`）に作っても読み込みません。**ワークスペース直下の1か所だけ**です
> - このフォルダは**AIには作れません**。`.ai-safety` への書き込みは安全ルールで禁止しているためで、AIに頼むと赤いブロック画面が出ます。Finder／エクスプローラーで自分で作ってください。**裏を返せば、AIが勝手にプラグインを仕込むことはできない**ということです
