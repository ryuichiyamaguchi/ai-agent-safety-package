...
> この経路にローカル LLM は不要です。パッケージ全体でローカル LLM を必要とする機能はありません（v1.17.0 で最後の 1 つを廃止しました）。
...
**これらは DeepSeek 送信検査 Gateway を通りません。** Gateway が見るのは DeepSeek へ送る文字列だけで、上の道具はそれぞれの相手（Google、Pollinations、Buffer）へ直接つながります。だから、どれも使うたびに確認画面が出るようにしてあります。**送ってよい内容かどうかは、その画面であなたが判断してください。**
...
- **送信検査 Gateway は「DeepSeek へ送る文字列」の検査です。** 検索・画像の読み取り・画像生成・Buffer・Web ページの取得は Gateway を通らず外へ出ます。そこは確認画面での判断があなたの仕事になります。
