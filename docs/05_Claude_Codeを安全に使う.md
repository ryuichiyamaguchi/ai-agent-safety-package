...
> **Claude Code は最新版に更新して使ってください。** 以前のこのパッケージは「パッケージが動作確認した版に合わせる」方式で版を固定していましたが、いまは**最新版追従**に変えました。Claude Code 純正の壁（サンドボックス）は新しい版でないと使えないためです。2 回目以降は、スタートフォルダの **「2_AIツールをまとめて入れる」** をダブルクリックすれば更新できます。
...
いちばん簡単なのは、作業フォルダの `スタート` フォルダにある **「4_AIを起動する」**（Mac は `.command`、Windows は `.bat`）を**ダブルクリック**して、メニューから Claude を選ぶ方法です。
...
> **導入（インストール）が作業フォルダを Claude の「信頼済み」に登録します。** この登録が無いと、Claude Code は権限の設定そのものを無視してしまいます。**別の案件を始めたくなったら**、`my-ai-workspace` の中にプロジェクトごとのサブフォルダを作って使ってください（信頼済み登録と安全設定がそのまま効きます）。
